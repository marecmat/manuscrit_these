...
\end{equation}
with $\mbf N$ containing the Lobatto shape functions, and $\mbf U$ is the vector of displacement amplitudes. Following the same discretization, the test function thus becomes $\forall \delta \mbf U$. By rearranging the terms of Eq.~\ref{eq:safe_rearranged_weak} and writing the weak form for one element, 
\begin{equation}
    \begin{aligned}
...
In Eq.~\ref{eq:safe_full_weakform}, the dependance on $k_1$ was highlighted and terms were sorted depending on their order. The elementary matrices should be assembled into the global matrices and then, following an analogous procedure to Chapter.~\ref{chap:jap}, the general form for the problem becomes,
\begin{equation}
