...
\section{Implementing SAFE to compute the dispersion relation of Lamb modes}

This section has initially been worked on as part of a letter to the reviewers of the paper\cite{marechal2025} dealing with the SCM and added to Chapter \ref{chap:chap1}. They wanted us to investigate the performance of SCM against that of SAFE, which we have done in a simple configuration, that of the free elastic plate. This work was carried out for the purpose of providing clarification for reviewers and was decided to not be included in the article. We provide it as the following appendix.
...
\end{equation}
and the double-dot product is $\mbf \sigma (u) : \mbf \varepsilon (\delta u ^\star) = \sigma_{ij}(u) \varepsilon_{ij}(\delta u ^\star)$. $\mbf \sigma$ designates the stress tensor while $\mbf \varepsilon$ is the strain tensor. Developing this part of the volume term gives the expression,
\begin{equation}
    \begin{aligned}
...
\end{equation}
with $\mbf N(\xi)$ containing the Lobatto shape functions that spans the local coordinate space $\xi \in [-1, 1]$ of a single element and $\mbf U$ is the vector of displacement amplitudes. Following the same discretization, the test function thus becomes $\forall \delta \mbf U$. By rearranging the terms of Eq.~\ref{eq:safe_rearranged_weak} and writing the weak form for one element, 
\begin{equation}
    \begin{aligned}
...
\end{equation}
This quadratic generalized eigenvalue problem can be linearized using companion linearization as, \cite{tisseur2001, strang2006}
\begin{equation}
    \left[\begin{pmatrix}
