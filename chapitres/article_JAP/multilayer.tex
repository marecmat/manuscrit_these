...
\label{chap:jap}
    ...
    {\color{gray}\textsc{This chapter is based on the material published in \cite{marechal2025}, authored by \textit{Mathieu Maréchal, Alan Geslain, Jean-Philippe Groby, Vicent Romero-Garc\'ia and Olivier Dazel}. Minor modifications to the text and figures have been made to fit the format and notation of this thesis}}\footnote{While the remaining of the manuscript assumes a positive harmonic time-dependance, the reader should bear in mind that this entire chapter is written using a negative time-dependance, i.e. $e^{-\im \omega t}$.}
...
\section{Foreword}
\label{sec:foreword}
There are fundamental differences between this chapter and the rest of the manuscript in how the leakage is treated in the approach. In this chapter, no questions were raised regarding the wave solutions in the surrounding media. As a first assumption, we considered that the leaky waves should be outgoing in order to satisfy some radiation condition as stated right before Eq.~\ref{eq:outgoing_pressure_field}. However, this matter is not so simple, and treated as part of the subsequent chapter. In that respect, the dispersion relation of the waves in this chapter show indiscriminately outgoing as well as incoming solution. Fortunately, in these configurations the solution spectrum in real parts of the transverse wavenumber are almost symmetrical for most cases, which is why we did not consider this aspect in this chapter. 
