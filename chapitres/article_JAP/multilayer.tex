...
While the remaining of the manuscript assumes a positive harmonic time-dependance, the reader should bear in mind that this entire chapter is written using a negative time-dependance, i.e. $e^{-\im \omega t}$.
...
\begin{figure}
    \centering
    \includegraphics{expe_setup.pdf}
...
\begin{figure}[ht]
    \centering
    \includegraphics{slatcow_annexe.pdf}
...
{The cost function $\mathfrak{C} = \left(\lVert \mathfrak L_{\text{rec}}(z) -\mathfrak L_{\text{mes}}(z) \rVert\right) / \lVert \mathfrak L_{\text{mes}}(z) \rVert$ is finally minimized using the Matlab function \textit{fminsearchbnd} for $\{|A^{\beta}|,~\phi^\beta,~\textrm{Re}\left( k^{\beta}\right),~\textrm{Im}\left( k^{\beta}\right) \}$, $\forall \beta$. The initial guess are the recovered values of the minimization parameters at the previous frequency, with the exception of the first frequency. The method is graphically exposed in Figure~\ref{fig:slatcow_annexe}. Figures ~\ref{fig:slatcow_annexe} c) and d) depict the complex dispersion relation as already depicted in Fig.~\ref{fig:expe}. Figure~\ref{fig:slatcow_annexe}a) depicts the spatial Laplace transform of the experimental data and Figure~\ref{fig:slatcow_annexe}b) depicts $\mathfrak L_{\text{rec}}(z)$ with the parameters derived from minimization of the cost function at the frequency $f = 3.56$ kHz. This frequency is highlighted by the purple dashed line in Figures ~\ref{fig:slatcow_annexe} c) and d). The two Laplace transforms match almost perfectly.}
\endgroup