...
Other variations of the FEM include the so-called $p$-FEM or higher-order finite elements, that uses polynomial Lobatto shape functions instead of the traditional Lagrange shape functions. Several works have highlighted it as being a very powerful discretization procedure \cite{beriot2016,sanghavi2021}. The \acrfull{wfe} \cite{}
+ dgm, bem and 


In this section, we propose a summary of the \acrfull{fem}, presented in the case of fluid domains. A simple derivation is done to describe generally the procedures involved in the method. Let us start with a simple Helmholtz equation governing the acoustic motion in a general domain $\Omega$, 
\begin{equation}
...
\end{equation}
where $\delta p$ is an admissible set of test functions associated to the pressure field. \comment{primal and dual here}
...
%Similarly, in poroelastic media governed by Biot equations, of the $(\mbf u_s, p)$ formulation \cite{atalla1998} has the weak form,
% \begin{equation}
%     \begin{aligned}
%         \int_{\Omega_p} \hat{\mbf \sigma_s}(\mbf u) : \mbf \epsilon_s(\delta \mbf u) d\Omega - \omega^2 \int_{\Omega_p} \tilde{\rho} \mbf u \delta \mbf u d\Omega - \int_{\Omega_p} \tilde \gamma \mbf \nabla p \delta \mbf u d\Omega - \int_{\partial \Omega_p} (\hat{\mbf \sigma_s}\cdot \mbf n)\delta \mbf u dS &= 0, \\
%         \int_{\Omega_p} \left(\frac{\phi^2}{\omega^2 \tilde\rho_{22}} \mbf \nabla p \mbf \nabla \delta p - \frac{\phi^2}{\tilde R}p \delta p\right) d\Omega - \int_{\Omega_p} \tilde \gamma \mbf \nabla \delta p \mbf u d\Omega + \int_{\partial \Omega_p} \left(\tilde \gamma u_n - \frac{\phi^2}{\tilde \rho_{22}\omega^2} \partial_n p \right) \delta p dS &= 0,\\
%         \forall (\delta \mbf u, \delta p).&\quad \quad \quad
%      \end{aligned}
% \end{equation}
